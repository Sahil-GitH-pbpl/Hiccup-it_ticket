...
from app.models import InfraTicket, InfraTicketImage, InfraUpdate, User
from app.utils.auth_guard import require_login, require_admin_designation
...
WHATSAPP_GROUP_TARGET = "120363141953100908@g.us"
...
def _build_ticket_message(ticket: InfraTicket) -> str:
    ...
    created_str = created_at.strftime("%d-%b-%Y %I:%M %p") if created_at else now_ist_naive().strftime("%d-%b-%Y %I:%M %p")
...
def _send_pick_reminders(db: Session):
    ...
    cutoff = now_ist_naive() - timedelta(hours=REMINDER_THRESHOLD_HOURS)
    pending = (
...
def _update_delayed_flags(db: Session):
    ...
    if delayed_tickets:
        for t in delayed_tickets:
            t.is_delayed_pick = True
...
async def create_ticket(
    request: Request,
    created_by: str = Form(...),   # ignored, we use session username
    department: str = Form(...),
    category: str = Form(...),
    ...
    if username:
        ...
        creator = (
            db.query(User)
            .filter(func.lower(User.name) == normalized_username)
            .first()
        )
        if creator and getattr(creator, "contact", None):
            user_contact = creator.contact.strip()
...
def all_tickets(
    ...
    check_admin = require_admin_designation(request)
    if check_admin:
        return check_admin
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        partial = templates.get_template("partials/tickets_block.html").render(context)
        return JSONResponse({"html": partial})

    return templates.TemplateResponse("all_tickets.html", context)
...
def pick_ticket(
    ...
    # Method 2: Quick commitment with predefined options
        ...
        try:
            ...
            ct = now_ist_naive().replace(second=0, microsecond=0)
    ...
    ticket.commitment_time = ct
    ticket.assigned_to = username
    ticket.status = "In Progress"

    # If pick itself is after commitment, mark delayed
    if now_ist_naive() > ct:
    ...
    update_note = f"Ticket picked by {username}. Commitment set to {ct.strftime('%d-%m-%Y %H:%M')}"
...
def resolve_ticket(
    ...
    if ticket.commitment_time and now_ist_naive() > ticket.commitment_time:
        ticket.is_delayed_pick = True
...
def mark_invalid(
    ...
    ticket.invalid_reason = invalid_reason.strip() or "Marked invalid by IT."
